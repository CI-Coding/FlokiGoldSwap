...
import useShortAmount from './useShortAmount'
...
const AccountElement = styled.div<{ active: boolean }>`
  ...
  flex-direction: row;
  align-items: center;
  background-color: ${({ theme, active }) => (!active ? theme.deprecated_bg1 : theme.deprecated_bg1)};
  border-radius: 8px;
  white-space: nowrap;
  width: 100%;
  height: 40px;
...
export default function Header() {
  ...
  const shortAmount = useShortAmount(
    userEthBalance?.toSignificant(30) ? parseFloat(userEthBalance?.toSignificant(30)) : 0,
    3
  )
          ...
          <AccountElement active={!!account}>
            {account && userEthBalance ? (
              <BalanceText style={{ flexShrink: 0, userSelect: 'none' }} pl="0.75rem" pr=".4rem" fontWeight={500}>
                <Trans>
                  {shortAmount} {nativeCurrencySymbol}
                </Trans>
              </BalanceText>
            ) : null}
            <Web3Status />
